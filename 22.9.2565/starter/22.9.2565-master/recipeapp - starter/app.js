...
const showdetail = async (id) => {
    ...
    // remove hidden  screen_recipe wii show detail food
    screen_recipe.classList.remove('hidden')

    let response = await fetch(`${API}lookup.php?i=52874`)
    let data = await response.json()
    console.log(data);
    let recipe = data.meals[0]
    console.log(recipe);
    screen_recipe.querySelector('.thumbnail img').scr = recipe.strMealThumb
    ...
    for (let index = 1; index < 20; index++) {
        ...
        li.innerText = `${recipe["strIngredient" + index]} - ${recipe["strMeasure"+index]}`
